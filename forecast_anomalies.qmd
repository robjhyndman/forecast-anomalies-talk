...
So $f(y_t | y_1,\dots,y_{t-1}) = \phi\left(\frac{y_t - \mu_t}{\sigma}\right)$
\end{textblock}
...
$M_n = \max \{Y_1, \dots, Y_n\}$ where $Y_1, \dots, Y_n \sim$ iid $F$.

Under some conditions, as $n\rightarrow\infty$,\newline $M_n$ converges in distribution to
\begin{align*}
  &\textbf{Weibull:} && \text{when $F$ has a finite upper bound (e.g., Uniform)} \\
  &\textbf{Gumbel:}  && \text{when $F$ has exponential tails (e.g., Normal or Gamma)} \\
  &\textbf{Fréchet:} && \text{when $F$ has heavy tails (e.g., Pareto or Weibull)}
\end{align*}
