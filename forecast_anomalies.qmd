...
\fontsize{13}{15}\sf
\begin{enumerate}\tightlist\itemsep=0.4cm
...
\item \textbf{Identify anomalies within a time series in real time}: use one-step forecast distributions
\item \only<2>{\textcolor{Orange}}{\textbf{Identify anomalies within a time series in historical data}: use residual distributions}
\item \textbf{Identify an anomalous time series in a collection of time series}: use feature-based approach
\end{enumerate}
\end{textblock}
